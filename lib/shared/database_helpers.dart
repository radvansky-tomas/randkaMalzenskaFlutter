...
final String photoPosition = 'position';
...
class DatabaseHelper {
  ...
  Future<int> insertOrUpdatePhoto(Photo photo) async {
    Database db = await database;
    int id;
    Photo? photoDb = await queryPhotoByPosition(photo.position!);
    if (photoDb != null) {
  ...
  Future<List<Photo>?> queryPhoto() async {
    ...
    List<Map> maps = await db.query(
      photoTableName,
      columns: [columnId, photoColumnPath, photoPosition],
    );
    List<Photo> photos = [];
    if (maps.length > 0) {
      maps.forEach((result) {
  ...
  Future<Photo?> queryPhotoByPosition(int position) async {
    Database db = await database;
    String whereString = '$photoPosition = ?';
    List<dynamic> whereArguments = [position];
